...
func (p *HuggingFace) doStreamChat(ctx context.Context, req *core.ChatRequest) (*core.ChatStream, error) {
	// Build model string with provider policy
	model := p.buildModelString(req.Model)

	// Build HF request with stream=true
	hfReq := buildRequest(req, model, true)
...
func (p *HuggingFace) processSSEStream(
	ctx context.Context,
	body io.ReadCloser,
