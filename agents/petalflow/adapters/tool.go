...
// It provides a simpler interface than tools.Tool for workflow integration.
type PetalTool interface {
	Name() string
	Invoke(ctx context.Context, args map[string]any) (map[string]any, error)
}

// ToolAdapter adapts a tools.Tool to the PetalTool interface.
...
// Useful for creating tools inline without implementing tools.Tool.
...
var (
	_ PetalTool = (*ToolAdapter)(nil)
	_ PetalTool = (*FuncTool)(nil)
)
