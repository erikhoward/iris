...
type ToolNode struct {
	...
	tool     adapters.PetalTool
	registry *adapters.ToolRegistry
}

// NewToolNode creates a new tool node with a specific tool.
func NewToolNode(id string, tool adapters.PetalTool, config ToolNodeConfig) *ToolNode {
	// Apply defaults
	if config.OutputKey == "" {
...
func NewToolNodeWithRegistry(id string, registry *adapters.ToolRegistry, config ToolNodeConfig) *ToolNode {
	// Apply defaults
	if config.OutputKey == "" {
		config.OutputKey = id + "_output"
	}
...
func (n *ToolNode) getTool() (adapters.PetalTool, error) {
