...
func TestNewToolNode(t *testing.T) {
	...
	if node.Kind() != NodeKindTool {
		t.Errorf("expected kind %q, got %q", NodeKindTool, node.Kind())
	}
}
...
func TestNewToolNode_DefaultOnError(t *testing.T) {
	...
	config := node.Config()
	if config.OnError != ErrorPolicyFail {
...
func TestToolNode_Run_WithRegistry(t *testing.T) {
	...
	registry.Register(adapters.NewFuncTool("multiply", "Multiplies numbers", func(ctx context.Context, args map[string]any) (map[string]any, error) {
		a := args["a"].(float64)
		b := args["b"].(float64)
