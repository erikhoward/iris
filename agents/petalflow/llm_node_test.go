...
type mockLLMClient struct {
	response adapters.LLMResponse
	err      error
	requests []adapters.LLMRequest // captures all requests
}

func (m *mockLLMClient) Complete(ctx context.Context, req adapters.LLMRequest) (adapters.LLMResponse, error) {
	m.requests = append(m.requests, req)
	if m.err != nil {
		return adapters.LLMResponse{}, m.err
	}
...
func TestLLMNode_Run_WithTemplate(t *testing.T) {
	client := &mockLLMClient{
		response: adapters.LLMResponse{Text: "Answer"},
	}
...
func TestLLMNode_Run_RetryOnError(t *testing.T) {
	...
	client := &countingMockLLMClient{
		failCount: 2,
		successResponse: adapters.LLMResponse{
			Text: "Success",
		},
		callCount: &callCount,
	}
...
func TestLLMNode_Run_Temperature(t *testing.T) {
	client := &mockLLMClient{
		response: adapters.LLMResponse{Text: "OK"},
	}
...
func (m *countingMockLLMClient) Complete(ctx context.Context, req adapters.LLMRequest) (adapters.LLMResponse, error) {
	*m.callCount++
	if *m.callCount <= m.failCount {
		return adapters.LLMResponse{}, errors.New("temporary failure")
	}
...
func (m *slowMockLLMClient) Complete(ctx context.Context, req adapters.LLMRequest) (adapters.LLMResponse, error) {
	select {
	case <-ctx.Done():
		return adapters.LLMResponse{}, ctx.Err()
	case <-time.After(m.delay):
		return adapters.LLMResponse{Text: "OK"}, nil
	}
}
