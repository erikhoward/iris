...
func NewLLMRouter(id string, client adapters.LLMClient, config LLMRouterConfig) *LLMRouter {
	if config.DecisionKey == "" {
		config.DecisionKey = id + "_decision"
	}
...
func (r *LLMRouter) parseResponse(resp adapters.LLMResponse) (RouteDecision, error) {
